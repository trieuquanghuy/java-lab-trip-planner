...
**Confidence:** MEDIUM-HIGH (all locked libraries verified against official sources or current release pages; some version pins rely on Maven Central / GitHub releases cross-checked with WebSearch)

---

> **Scope of this document:** The stack is already locked by user choice (see `docs/02-architecture.md`).
> This document validates each choice against 2026 reality, flags version gotchas, and surfaces
> 2026-fresh alternatives as advisories only. No stack changes are recommended.
...
**CRITICAL GOTCHA — Spring Boot 3.3 is EOL:**
Spring Boot 3.3.x OSS support ended June 30, 2025. No further CVE patches will be issued for that line. The project spec says "3.3.x" but should be started on **3.5.x** instead. Spring Boot 3.4 is also EOL (Dec 31, 2025). Spring Boot 3.5.x receives patches until June 30, 2026, which covers the entire expected build window for this portfolio project. Upgrading to 3.5.x from 3.3.x is low-friction (same Spring Framework 6 baseline).
...
**Advisory — Eureka alternatives:** Consul (HashiCorp, fully maintained), Kubernetes-native service discovery (env-based or CoreDNS). Neither is worth switching to for this portfolio project.

**SECURITY ADVISORY — Spring Cloud Gateway (CVE-2025-41235, CVE-2025-41253):**
Spring Cloud Gateway 4.1.x/earlier had two CVEs in 2025: forwarding headers from untrusted proxies, and environment modification. Both are fixed in the 4.2.x patch releases of the 2024.0 train. Upgrading to Spring Cloud 2024.0.x (with Spring Boot 3.5) resolves both.
...
**Advisory — Spring Security's built-in JWT support (2026 alternative):**
Spring Security 6.x (included in SB 3.5) bundles full JWT support via `spring-boot-starter-oauth2-resource-server` using the Nimbus JOSE+JWT library, with no external dependency. This eliminates jjwt entirely. However, ADR-5 explicitly chose jjwt for simplicity — it remains a valid choice. Note this option for v2 when OAuth/OIDC login may be added.
...
# Spring
springBoot = "3.5.14"
springCloud = "2024.0.4"  # matches Spring Boot 3.5.x

# Persistence
postgresql = "42.7.x"  # JDBC driver (latest 42.7 series)
flyway = "10.x"        # managed by Spring Boot; add flyway-database-postgresql explicitly
hibernate = "6.6.x"    # managed by Spring Boot
...
wiremock = "3.9.0"  # org.wiremock.integrations:wiremock-spring-boot

[versions.frontend]
# Frontend (package.json)
react = "^18.3.1"
typescript = "^5.8.0"
vite = "^6.x"          # or 5.x if starting conservative; 6.x preferred for new project
vitest = "^3.x"        # matches Vite 5 or 6
tanstack-query = "^5.100.0"
zustand = "^5.0.0"
react-hook-form = "^7.55.0"
zod = "^3.24.0"        # stay on v3 to match locked spec; v4 is a notable future upgrade
axios = "^1.16.0"      # must be >= 1.15.0 due to CVEs
dnd-kit-core = "^6.x"  # @dnd-kit/core — do NOT use @dnd-kit/react (pre-1.0)
leaflet = "^1.9.0"
react-leaflet = "^4.2.x"  # v4 for React 18; v5 requires React 19
tailwindcss = "^3.4.x"
playwright = "^1.59.0"
```
...
*Stack research for: Trip Planner — Spring Boot microservices + React SPA*
*Researched: 2026-05-08*
