...
- **D-24 (Account-enumeration on `/signup` — locked):** `POST /api/auth/signup` ALWAYS returns opaque `201 Created {userId}` for both new accounts AND existing-and-verified accounts (where `userId` is the existing user's id). The `auth.email_already_registered` error code stays in the catalog (`docs/04 §6`) for future admin endpoints but MUST NOT be emitted from `/signup`. UI-SPEC §Server-Driven Copy Contract row for `auth.email_already_registered` is marked "NOT EMITTED" — this is the locked behavior. Resolves RESEARCH.md Open Question #5 (Option A); matches `docs/05 §9.1` enumeration policy verbatim. Rationale: lockstep response shape regardless of email-existence prevents account-enumeration via signup; `docs/04 §6` is the catalog of *possible* codes, not the catalog of *signup-emitted* codes.
