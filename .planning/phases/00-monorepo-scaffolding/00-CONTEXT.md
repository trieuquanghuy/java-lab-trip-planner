...
### Plan-Phase Addendum (2026-05-08)
Decisions confirmed during `/gsd-plan-phase 0` after research surfaced corrections and open questions. Treat these as locked alongside D-01..D-29 above.

- **D-30: Spring Cloud train is `2025.0.x` (Northfields), NOT `2024.0.x`.** Researcher verified against three spring.io release announcements: Spring Cloud 2024.0.x is built on Spring Boot 3.4 and is incompatible with our locked Spring Boot 3.5.x; Spring Cloud 2025.0.x (Northfields) is the matching train for SB 3.5 (latest patch as of April 2026 is 2025.0.2). `gradle/libs.versions.toml` MUST pin `spring.cloud.version = "2025.0.2"` (or the latest 2025.0.x patch). **Note:** `CLAUDE.md` cites 2024.0.x — that pin is incorrect and will be updated separately; the correct value for this project is 2025.0.x.
- **D-31: Frontend uses Vite 6.x + Vitest 3.x or 4.x** (overrides D-12's "Vite 5 or 6"). Pick latest stable patch. Vitest 4 requires Vite ≥ 6 and Node ≥ 20 — both satisfied. Vite 6 breaking changes (Sass API default, build target name, Runtime API rename) are noise-level for a greenfield Phase 0 with no migration burden.
- **D-32: shadcn CLI invocation is `pnpm dlx shadcn@latest init`** with explicit prompt answers selecting Tailwind v3 + React 18 (NOT the v4 + React 19 default). Document the exact prompt answers in `frontend/README.md` under "Adding components" so Phase 7 reproduces the same path. The `components.json` written by init is the source of truth — once landed, future component adds (`shadcn@latest add <component>`) read from it and respect the v3 path.
- **D-33: `scripts/smoke.sh` is a Wave 1 deliverable** (NOT deferred to a final wave). The script enforces all 5 ROADMAP success criteria (compose healthy <60s, all 4 services in Eureka, gateway `/actuator/health` UP, frontend renders, per-service Flyway history tables present). Landing it early means each subsequent wave's containers can be smoke-tested incrementally as they come online; it also doubles as the verification harness for the phase verifier in `/gsd-verify-phase`. Smoke script lives at `scripts/smoke.sh`, is executable, exits non-zero on any criterion failure, and prints which criterion failed.
